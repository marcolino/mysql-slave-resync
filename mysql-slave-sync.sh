#!/usr/bin/env bash
#
# Script to resync a MySQL slave replication database.
...
SLAVE_HOST="new.mypass.cc"
SLAVE_USER="marco"
SLAVE_PASS="alma-CORE-m4g4!"
DBS="$*"
TMP="/tmp/mysql-sync-slave"
...
\""
  ...
  #su - "$SLAVE_USER" -c "ssh \"${SLAVE_USER}@${SLAVE_HOST}\" \"\
#zcat /tmp/${DB}.sql.gz | mysql -u root -p'${SLAVE_PASS}' --max_allowed_packet=500M "$DB"
  su - "$SLAVE_USER" -c "ssh \"${SLAVE_HOST}\" \"\
